...
void ReloadFonts(void)
{
    ...
    *fontTexture = LoadTextureFromImage(image);
    UnloadImage(image);
    io->Fonts->TexID = (uintptr_t)fontTexture;
}

static const KeyboardKey RaylibKeys[] = {KEY_APOSTROPHE,
                                         KEY_COMMA,
                                         KEY_MINUS,
                                         KEY_PERIOD,
                                         KEY_SLASH,
...
void SetupBackend(void)
{
    ...
    io->BackendFlags |=
        ImGuiBackendFlags_HasMouseCursors | ImGuiBackendFlags_HasGamepad | ImGuiBackendFlags_HasSetMousePos;
...
void rligSetupFontAwesome(void)
{
    ...
    ImGuiIO *io = igGetIO();
    ImFontAtlas_AddFontFromMemoryCompressedTTF(io->Fonts, (void *)fa_solid_900_compressed_data,
    ...
    for (int l = 0; l < draw_data->CmdListsCount; ++l)
    {
        ...
        for (int ci = 0; ci < commandList->CmdBuffer.Size; ++ci)
        {
            ...
            ImGuiRenderTriangles(cmd.ElemCount, cmd.IdxOffset, &commandList->IdxBuffer, &commandList->VtxBuffer,
                                 (Texture2D *)(uintptr_t)cmd.TextureId);
            rlDrawRenderBatchActive();
        }
    }
...
void rligImageRenderTextureFit(const RenderTexture *image, bool center)
{
    ...
    rligImageRect(&image->texture, sizeX, sizeY,
                  (Rectangle){0, 0, (float)(image->texture.width), -(float)(image->texture.height)});
}
...
bool rligImageButton(const char *name, const Texture *image)
{
    ...
    return igImageButton(name, (ImTextureID)image, (ImVec2){(float)(image->width), (float)(image->height)}, uv, uv,
                         color, color);
}
